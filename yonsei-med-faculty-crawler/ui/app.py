...
if not DB_PATH.exists():
    st.error("DB 파일이 없습니다. 먼저 `python run.py` 또는 Docker 크롤링 명령으로 데이터를 수집해 주세요.")
    st.stop()
...
m2.metric("이메일 보유", int(result_df["email"].fillna("").astype(str).str.strip().ne("").sum()) if not result_df.empty else 0)
m3.metric("전화 보유", int(result_df["phone"].fillna("").astype(str).str.strip().ne("").sum()) if not result_df.empty else 0)
