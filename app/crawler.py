...
EXTERNAL_SEARCH_PATHS = list(dict.fromkeys(getattr(app_config, "EXTERNAL_SEARCH_PATHS", [
...
DEFAULT_EXTERNAL_PROFILE_SOURCES = [
    {
        ...
        "search_paths": list(EXTERNAL_SEARCH_PATHS),
    },
    {
...
EXTERNAL_SOURCE_MAX_PAGES = getattr(app_config, "EXTERNAL_SOURCE_MAX_PAGES", 12)
EXTERNAL_SOURCE_MAX_HOPS = getattr(app_config, "EXTERNAL_SOURCE_MAX_HOPS", 1)
...
def load_external_profile_sources() -> List[Dict[str, str]]:
    configured_sources = getattr(app_config, "EXTERNAL_PROFILE_SOURCES", None)
        ...
        try:
            ...
            if isinstance(sources, list) and sources:
                ...
                for source in sources:
                    ...
                    normalized["search_paths"] = _dedupe_preserve_order(
                        list(normalized.get("search_paths") or [])
                    ) or list(EXTERNAL_SEARCH_PATHS)
                    normalized_sources.append(normalized)
                if normalized_sources:
                    return normalized_sources
        except Exception:
            logging.warning("failed to load external source config", exc_info=True)
...
def _normalize_text_for_contains(text: str) -> str:
    return re.sub(r"\s+", " ", clean_text(text or "")).strip().lower()


def _extract_from_href_prefix(html: str, prefixes: Tuple[str, ...]) -> str:
    ...
    for a in soup.find_all("a", href=True):
        ...
        for prefix in prefixes:
            if href.lower().startswith(prefix.lower()):
                ...
                if prefix.lower().startswith("mailto"):
                    return normalize_email(value)
                if prefix.lower().startswith("tel"):
                    return normalize_phone(value)
    return ""


def _extract_labeled_multi(lines: List[str], labels: List[str]) -> str:
    normalized = []
    for line in lines:
        normalized.append((line, clean_text(line).replace("：", ":")))
    for raw_line, line in normalized:
        for label in labels:
...
def _extract_name_ko_fallback(lines: List[str]) -> str:
    for line in lines:
        m = NAME_KO_INLINE_RE.search(line)
        if m:
            value = clean_text(m.group(1))
            if value not in NOISE_LINES:
                return value
    return ""


def _href_keyword_score(url: str) -> int:
    url_l = (url or "").lower()
    score = 0
    for keyword, weight in [
        ("profile", 25), ("professor", 20), ("faculty", 20), ("doctor", 18),
        ("staff", 12), ("people", 12), ("member", 12), ("medstaff", 20),
        ("specialist", 14), ("clinic", 8), ("find", 8), ("search", -10),
        ("board", -4), ("notice", -15), ("news", -12),
        ("의료진", 25), ("교수", 20), ("교원", 18), ("교실", 12), ("진료", 10),
    ]:
        if keyword in url_l:
            score += weight
    return score
...
def _text_token_overlap_score(text: str, tokens: List[str]) -> int:
    blob = _normalize_text_for_contains(text)
    score = 0
    for token in tokens:
        token_n = _normalize_text_for_contains(token)
        if token_n and token_n in blob:
            score += 10
    return score


def _extract_candidate_links_from_html(html: str, base_url: str, queries: List[str]) -> List[str]:
    soup = BeautifulSoup(html or "", "html.parser")
    candidates = []
    seen = set()
    for a in soup.find_all("a", href=True):
        ...
        if not _same_domain(href, base_url):
            continue
        score = _href_keyword_score(href) + _text_token_overlap_score(blob, queries)
        if score < 10:
            continue
        if href not in seen:
            seen.add(href)
...
def _collect_external_candidate_urls(session: Session, source: Dict[str, str], queries: List[str], max_urls: int = EXTERNAL_SOURCE_MAX_PAGES) -> List[str]:
    ...
    search_paths = source.get("search_paths") or EXTERNAL_SEARCH_PATHS
    candidates = [(5, base_url)]
    seen = {base_url}
        ...
        for path in search_paths:
            ...
            try:
                ...
                if resp.status_code >= 400:
                    continue
                for href in _extract_candidate_links_from_html(resp.text, search_url, queries):
                    if href not in seen:
                        seen.add(href)
                        candidates.append((_href_keyword_score(href) + 20, href))
    ...
    try:
        ...
        if resp.status_code < 400:
            for href in _extract_candidate_links_from_html(resp.text, base_url + "/", queries):
                if href not in seen:
                    seen.add(href)
                    candidates.append((_href_keyword_score(href), href))
                if len(candidates) >= max_urls:
                    break
    except Exception:
        pass
    ...
    candidates.sort(key=lambda x: x[0], reverse=True)
    return [href for _, href in candidates[:max_urls]]
...
def _parse_external_profile_html(html: str) -> Dict[str, str]:
    ...
    email = (
        _extract_labeled_multi(lines, ["E-mail", "Email", "메일", "이메일"]) or
        _extract_from_href_prefix(html or "", ("mailto:",)) or
        _extract_email_fallback(html or "", lines)
    )
    phone = (
        _extract_labeled_multi(lines, ["Tel", "Phone", "전화", "연락처", "Office Tel"]) or
        _extract_from_href_prefix(html or "", ("tel:",)) or
        _extract_phone_fallback(lines)
    )
    office = (
        _extract_labeled_multi(lines, ["Office", "연구실", "사무실", "위치", "주소", "진료실"]) or
        _extract_office_fallback(lines)
    )
    ...
    name_ko = _extract_name_ko_fallback(lines)
    name_en = _extract_name_en_fallback(lines)
    orcid_id, orcid_url = _extract_orcid_from_text(html or "")
...
def enrich_from_external_sources(
    ...
    try:
        for source in EXTERNAL_PROFILE_SOURCES:
            seed_urls = _collect_external_candidate_urls(session, source, queries)
            inspect_urls = []
            seen_urls = set()
            ...
            hop = 0
            while inspect_urls and hop <= EXTERNAL_SOURCE_MAX_HOPS:
                current_urls = inspect_urls[:max(1, EXTERNAL_SOURCE_MAX_PAGES)]
                inspect_urls = []
                for candidate_url in current_urls:
                    try:
                        ...
                        page_score = (
                            ...
                            + _href_keyword_score(candidate_url)
                            + _text_token_overlap_score(html, queries)
                        )
                        parsed = _parse_external_profile_html(html)
                        if not getattr(record, "name_en", "") and parsed.get("name_en"):
                            page_score += 20
                        if not getattr(record, "email", "") and parsed.get("email"):
                            page_score += 30
                        if not getattr(record, "phone", "") and parsed.get("phone"):
                            page_score += 10
                        if not getattr(record, "office", "") and parsed.get("office"):
                            page_score += 10
                        if parsed.get("name_ko") and getattr(record, "name_ko", "") and parsed.get("name_ko") == getattr(record, "name_ko", ""):
                            page_score += 40
                        if page_score >= 70:
                            ...
                            if page_score > best_score:
                                ...
                                best_data = parsed
                        elif hop < EXTERNAL_SOURCE_MAX_HOPS:
                            for href in _extract_candidate_links_from_html(html, candidate_url, queries)[:6]:
                                if href not in seen_urls:
                                    seen_urls.add(href)
                                    inspect_urls.append(href)
                    except Exception:
                        continue
                hop += 1
        ...
        if best_data:
            ...
            if best_data.get("source_url"):
                try:
                    setattr(record, "external_source_url", best_data.get("source_url", ""))
                except Exception:
                    pass
...
def _purge_stale_chromedriver_cache():
    cache_roots = [
        Path.home() / ".wdm",
    ...
    for root in cache_roots:
        try:
            if root.exists():
                ...
                logging.warning("purged stale driver cache: %s", root)
        except Exception:
            logging.warning("failed to purge driver cache: %s", root, exc_info=True)


def _is_probably_executable_driver(path: Path) -> bool:
    name = path.name.lower()
    if not path.is_file():
        return False
    if _is_windows():
        return name == "chromedriver.exe"
    return name == "chromedriver"


def _select_local_chromedriver() -> str:
    candidates = []
    ...
    for root in local_roots:
        ...
        if root.is_file():
            if _is_probably_executable_driver(root):
                return str(root)
            continue

        pattern = "chromedriver.exe" if _is_windows() else "chromedriver"
        for candidate in root.rglob(pattern):
            if _is_probably_executable_driver(candidate):
...
def _resolve_chromedriver_path() -> str:
    driver_path = _select_local_chromedriver()

    if not driver_path:
        driver_path = ChromeDriverManager().install()
        path_obj = Path(driver_path)

        if not _is_probably_executable_driver(path_obj):
            sibling_name = "chromedriver.exe" if _is_windows() else "chromedriver"
            sibling = path_obj.with_name(sibling_name)
            if sibling.exists() and _is_probably_executable_driver(sibling):
                driver_path = str(sibling)

    path_obj = Path(driver_path)
    if _is_probably_executable_driver(path_obj):
        return str(path_obj)

    logging.warning("invalid chromedriver candidate detected: %s", driver_path)
    _purge_stale_chromedriver_cache()
    ...
    if not _is_probably_executable_driver(path_obj):
        raise RuntimeError(
            f"chromedriver 실행 파일을 찾지 못했습니다. 선택된 경로: {driver_path}"
        )
...
def make_driver(headless: bool = HEADLESS):
    ...
    logging.info("selected chromedriver: %s", driver_path)

    try:
        service = Service(executable_path=driver_path)
        driver = webdriver.Chrome(service=service, options=options)
    except OSError as e:
        if getattr(e, "winerror", None) == 193 or "WinError 193" in str(e):
            raise RuntimeError(
                "chromedriver 실행 실패: 올바른 .exe 파일이 아니거나 "
                "잘못된 파일(예: LICENSE/THIRD_PARTY_NOTICES)이 선택되었을 수 있습니다. "
                f"선택 경로: {driver_path}"
            ) from e
        raise
...
def safe_get(driver, url: str, retries: int = 2, wait_timeout: int = 20):
    last_error = None
    max_attempts = max(1, retries)

    for attempt in range(1, max_attempts + 1):
        try:
            logging.info("safe_get attempt %d/%d: %s", attempt, max_attempts, url)
            driver.get(url)
            ...
            try:
                wait_document_ready(driver, timeout=min(wait_timeout, 15))
            except Exception:
                pass
            ...
            logging.warning(
                "safe_get failed %d/%d: %s | %r | retry in %ss",
                attempt,
                max_attempts,
                url,
                e,
...
def _crawl_department_with_retry(
    driver,
    dept,
    ...
    for attempt in range(1, max_attempts + 1):
        if progress:
            ...
            progress.emit(
                20,
                f"{dept['department_ko']} 재시도 {attempt}/{max_attempts}"
            )
        try:
            return parse_department_page(
                driver,
                dept,
            ...
            last_error = e
            logging.warning(
                "department retry failed %s (%d/%d): %r",
                dept.get("department_ko", ""),
                attempt,
                max_attempts,
                e,
            )
...
def crawl_department_chunk(
    ...
    try:
        ...
        for idx, dept in enumerate(departments, start=1):
            ...
            try:
                dept_records = _crawl_department_with_retry(
                    driver,
                    dept,
                    existing_detail_urls,
                    retries,
                    wait_timeout,
                    recrawl,
                    progress.cancel_check if progress else None,
                    progress,
                    orcid_token,
                    enable_external_enrichment,
                    session,
