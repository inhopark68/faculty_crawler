...
def _normalize_name_tokens(name: str) -> List[str]:
    ...
    return sorted([re.sub(r"[^A-Za-z]", "", p).lower() for p in parts if p.strip()])


def _english_name_match_score(source_name: str, target_name: str) -> int:
    src_norm = _normalize_name_for_match(source_name)
    tgt_norm = _normalize_name_for_match(target_name)
    if src_norm and tgt_norm and src_norm == tgt_norm:
        return 60

    src_tokens = _normalize_name_tokens(source_name)
    tgt_tokens = _normalize_name_tokens(target_name)
...
def _profile_context_score(text: str) -> int:
    text_l = (text or "").lower()
    score = 0

    positive_keywords = [
        "교수", "부교수", "조교수", "임상교수", "명예교수",
        "의료진", "진료과", "연구실",
        "professor", "faculty", "doctor", "office", "lab",
    ]
    negative_keywords = [
        "공지", "뉴스", "기사", "보도", "채용",
        "notice", "news", "article", "press", "search result",
    ]
...
def _page_identity_score(text: str, name_ko: str, name_en: str, email: str, department_ko: str, department_en: str) -> int:
    ...
    has_name_ko = bool(name_ko and name_ko in text)
    has_name_en = bool(_english_name_match_score(name_en, text) > 0)
    has_email = bool(email and normalize_email(email) and normalize_email(email) in normalize_email(text))
    has_dept_ko = bool(department_ko and department_ko in text)
    has_dept_en = bool(department_en and department_en.lower() in text.lower())

    if has_name_ko:
        score += 50
    if has_name_en:
        score += _english_name_match_score(name_en, text)
    if has_email:
        score += 120
    if has_dept_ko:
        score += 25
    ...
    if (has_name_ko or has_name_en) and (has_dept_ko or has_dept_en):
        score += 40

    if has_email and (has_name_ko or has_name_en):
        score += 60
                ...
                for candidate_url in current_urls:
                    try:
                        ...
                        if parsed.get("name_ko") and getattr(record, "name_ko", "") and parsed.get("name_ko") == getattr(record, "name_ko", ""):
                            page_score += 40
                        existing_email = normalize_email(getattr(record, "email", ""))
                        parsed_email = normalize_email(parsed.get("email", ""))
                        has_email_match = bool(existing_email and parsed_email and existing_email == parsed_email)
                        threshold = 70 if has_email_match else 95
...
def make_driver(headless: bool = HEADLESS):
    ...
    local_candidates = [
        Path.home() / ".wdm" / "drivers" / "chromedriver" / "win64" / "146.0.7680.165" / "chromedriver-win32" / "chromedriver.exe",
        Path.home() / ".wdm" / "drivers" / "chromedriver" / "win64" / "146.0.7680.165" / "chromedriver.exe",
    ]
    ...
    for candidate in local_candidates:
        if candidate.exists():
            ...
            logging.info("use cached chromedriver: %s", driver_path)
            break
